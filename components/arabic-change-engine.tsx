...
import {
  arabicLineUtils,
  OptimizedChangeRequest,
  characterDiffUtils,
  FullTextChangeRequest,
} from "@/lib/line-based-diff";
...
const arabicUtils = {
  ...
  detectArabicChanges: (
    ...
    const originalMap = new Map(
      originalChars.map((char, index) => [index, char])
    );
    const modifiedMap = new Map(
      modifiedChars.map((char, index) => [index, char])
    );
      ...
      if (originalChar !== modifiedChar) {
        ...
        if (originalNode && modifiedNode) {
          changes.push({
            ...
            changeType: arabicUtils.isTashkeel(modifiedChar)
              ? "tashkeel"
              : "main",
            ...
            context: {
              ...
              surroundingWords: original
                .split(/\s+/)
                .filter(
                  (_, idx) => Math.abs(idx - originalNode.wordNumber) <= 1
                ),
            },
          });
          ...
          changes.push({
            ...
            context: {
              ...
              surroundingWords: modified
                .split(/\s+/)
                .filter(
                  (_, idx) => Math.abs(idx - modifiedNode.wordNumber) <= 1
                ),
            },
          });
        }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="text-center">
          <h1 className="text-4xl font-bold text-slate-800 mb-2">
            {isAdminMode
              ? "Admin Panel - Review Changes"
              : "Arabic Text Editor"}
          </h1>
          <p className="text-slate-600">
            Page {pageData.pageNumber} • Book {pageData.bookId}
          </p>
        </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="text-center p-3 bg-blue-50 rounded-lg">
                    <div className="text-2xl font-bold text-blue-600">
                      {chars.reduce(
                        (acc, char) =>
                          acc +
                          char.suggestions.filter((s) => s.status === "pending")
                            .length,
                        0
                      )}
                    </div>
                  ...
                  <div className="text-center p-3 bg-green-50 rounded-lg">
                    <div className="text-2xl font-bold text-green-600">
                      {chars.reduce(
                        (acc, char) => acc + char.changeHistory.length,
                        0
                      )}
                    </div>
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
            <CardHeader>
              ...
              <p className="text-sm text-slate-500">
                Click on highlighted characters to review and approve/decline
                changes
              </p>
            </CardHeader>
            <CardContent>
